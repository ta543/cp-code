// time-limit: 1000
// problem-url: https://codeforces.com/contest/1951/problem/B
//
...
void solve(int tc) {
    ...
    vi nextSmallerOrEqual(N + 1, N);
    FOR_R(i, N) {
        nextSmallerOrEqual[i] = (A[i] <= A[K]) ? nextSmallerOrEqual[i + 1] : i;
    }
    int maxSoFar = 0, answer = 0;
    FOR(i, N) {
        if (A[K] >= maxSoFar && (i <= K || A[K] > A[i])) {
            int j = nextSmallerOrEqual[i + 1];
            if (i < K && A[i] > A[K]) {
                j = min(j, K);
            }
            answer = max(answer, static_cast<int>(j - i - 1 + (i > 0)));
        }
