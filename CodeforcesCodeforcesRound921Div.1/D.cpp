// time-limit: 1000
// problem-url: https://codeforces.com/contest/1924/problem/D
// 
...
mi at_least(int N, int M, int mn) {
    if (mn > 0) return 0;
    if (mn > N - M) return 0;
    --mn;
    mi res = F.C(N + M, M) - F.C(N + M, M + mn);
    return res;
}

mi solve(int N, int M, int K) {
    return at_least(N, M, K - M) - at_least(N, M, K - M + 1);
}
