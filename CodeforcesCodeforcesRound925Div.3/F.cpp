...
#pragma GCC unroll(4)
    ...
    if (lv == -1) return n;
#pragma GCC unroll(3)
    for (int h = lv - 1; h >= 0; h--) { i *= B, i += lowbit(seg[h][i / B]); }
    return i;
  }
...
#pragma GCC unroll(4)
    for (int h = 0; h < log; h++) {
      ...
      i -= __builtin_clzll(d), lv = h;
      break;
    }
    if (lv == -1) return -1;
#pragma GCC unroll(3)
    for (int h = lv - 1; h >= 0; h--) { i *= B, i += topbit(seg[h][i / B]); }
    return i;
  }
...
void solve() {
  LL(K, N);
  VV(int, A, N, K);
  FOR(i, N) FOR(j, K) A[i][j]--;

  Graph<int, 1> G(K);
  FOR(i, N) FOR(k, 1, K - 1) { G.add(A[i][k], A[i][k + 1]); }
  G.build();
