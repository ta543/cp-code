// time-limit: 2000
// problem-url: https://codeforces.com/contest/1929/problem/F
...
template <auto &os, bool debug, bool print_nd> struct Writer {
    ...
    void print_with_sep(const string &sep, F const &f,
                        Ts const &...ts) const {
        ImplWrapper(f), ((os << sep, ImplWrapper(ts)), ...), os << '\n';
    }
    void print_with_sep(const string &) const { os << '\n'; }
};
...
void solve() {
    ...
    for (int i = 1; i < v.size(); i++) {
        int c = v[i].second - v[i - 1].second + 1;
        int m = v[i].first - v[i - 1].first - 1;
        for (int x = 1; x <= m; x++) {
            ans *= m + c - x;
            den *= x;
        }
