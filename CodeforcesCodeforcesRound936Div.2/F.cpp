// time-limit: 6000
// problem-url: https://codeforces.com/contest/1946/problem/F
//
...
void solve(int tc) {
    ...
    fenwick<uint64_t> fenw(N);
    vector<uint64_t> add(N + 1);
    vector<int> who;
    FOR_R(i, N) {
      int X = A[i];
      add[X] = 1;
      FOR(V, X, N + 1, X) {
        fenw.modify(pos[V], add[V]);
        FOR(U, V + V, N + 1, V) {
          if (pos[V] < pos[U]) {
            add[U] += add[V];
          }
