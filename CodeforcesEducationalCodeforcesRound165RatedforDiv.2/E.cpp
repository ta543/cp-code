// time-limit: 2000
// problem-url: https://codeforces.com/contest/1969/problem/E
//
...
struct Info {
    int min = 0;
    void apply(const Tag &t) & { min += t.add; }
};
...
struct LazySegmentTree {
    ...
    template<class T>
    LazySegmentTree(vector<T> init_) { init(init_); }
    void init(int n_, Info v_ = Info()) { init(vector(n_, v_)); }
    template<class T>
    void init(vector<T> init_) {
        n = sz(init_);
        int size = 4 * (1 << (int)(ceil(log2(n))));
...
void solve(int tc) {
    def(int, n);
    vi a(n), fst(n, -1), prv(n, -1), nxt(n, n);
    FOR(i, n) {
        re(a[i]);
        --a[i];
    }
    ...
    FOR(x, n) {
        if (fst[x] != -1)
            seg.rangeApply(fst[x], nxt[fst[x]], {1});
    }

    vi f(n, n);
    FOR(l, n) {
        f[l] = seg.findFirst(l, n, [](const Info &info) { return info.min == 0; });
        if (f[l] == -1) f[l] = n;
        seg.rangeApply(l, nxt[l], {-1});
        if (nxt[l] < n)
            seg.rangeApply(nxt[l], nxt[nxt[l]], {1});
    }
