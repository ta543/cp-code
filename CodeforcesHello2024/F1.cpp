...
struct Info {
    ll v0, change, ans;
    pl eval(ll x) const {
        if (x <= change) return {v0, ans + x};
        return {v0 + x - change, ans + change};
    }
    pl eval(pl p) const {
        auto q = eval(p.f);
        return {q.f, q.s + p.s};
...
ostream &operator<<(ostream &os, const Info &i) {
    os << "[" << i.v0 << " " << i.change << " " << i.ans << "]";
    return os;
}
 
Info operator+(const Info &l, const Info &r) {
    auto [v0, ans] = r.eval({l.v0, l.ans});
    ll change = max(l.change, r.change + l.change - l.v0);
    Info i{v0, change, ans};
    return i;
}
...
int main() {
    setIO();
    def(int, N, Q);
    A.rsz(N), B.rsz(N), C.rsz(N - 1);
    re(A, B, C);
    seg.init(N);
    auto update = [&](int i) {
        seg.upd(i, Info{A[i], 0, 0} + Info{0, B[i], 0});
    };
    F0R(i, N) update(i);
    dbg(seg.seg.at(1));
    rep(Q) {
