// time-limit: 3000
// problem-url: https://codeforces.com/contest/1930/problem/F
...
void solve(int tc) {
  ...
  vi possible(1 << p);
  int last = 0;
  auto set_submasks = y_combinator([&](auto self, int x, int m) -> void {
    if (possible[x] & m) return;
    possible[x] |= m;
    if (possible[x] == 3) ckmax(last, x);
    F0R(i, p) if (x & (1 << i)) self(x ^ (1 << i), m);
  });
  vi answers;
  F0R(i, Q) {
    e.at(i) = (e.at(i) + last) % N;
    set_submasks(e.at(i), 1);
    set_submasks((1 << p) - 1 - e.at(i), 2);
    answers.pb(last);
  }
  ps(answers);
}
